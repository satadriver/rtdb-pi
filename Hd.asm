...
szReadMainError                                 db 'Read Main Error!',0dh,0ah,0
szLaunchReadError                               db 'Launch Read Error!',0dh,0ah,0
...
szGuardPageViolation                            db '%s缓冲区即将溢出,请迅速找到解决方法-->'
                                                db '异常地址:%08x,异常错误码:%08x,异常标志位:%08x!',0dh,0ah,0 
...
szExceptionMemUnroll                            db '注意:缓冲区地址空间已经不够,指针将回绕,缓冲区数据将会因为被覆盖而丢失',0dh,0ah,0                          
...
_dwRecordingBackupBufSize       dd 0
...
invoke VirtualAlloc,NULL,_dwRecordingBackupBufSize,MEM_RESERVE,PAGE_NOACCESS
        .if eax==0
        invoke WriteFile,_hFileJournal,addr szMemAllocError,sizeof szMemAllocError-1,addr @dwCounter,0
        call _ProcTerminate
        .endif
mov _lpRecordingBuf,eax
invoke VirtualAlloc,_lpRecordingBuf,_dwRecordingBufSize,MEM_COMMIT,PAGE_READWRITE
        .if eax==0
        invoke WriteFile,_hFileJournal,addr szMemAllocError,sizeof szMemAllocError-1,addr @dwCounter,0
        call _ProcTerminate
        .endif
...
sub ebx,ecx
invoke VirtualProtect,ebx,ecx,PAGE_READWRITE OR PAGE_GUARD,addr @flOldProtect
...
invoke VirtualAlloc,_lpRecordReservedBuf,_dwRecordingBufSize,MEM_COMMIT,PAGE_READWRITE
        .if eax==0
        invoke WriteFile,_hFileJournal,addr szMemAllocError,sizeof szMemAllocError-1,addr @dwCounter,0
        call _ProcTerminate
        .endif
...
invoke VirtualAlloc,NULL,_dwIndexBackupBufSize,MEM_RESERVE,PAGE_NOACCESS
        .if eax==0
        invoke WriteFile,_hFileJournal,addr szMemAllocError,sizeof szMemAllocError-1,addr @dwCounter,0
        call _ProcTerminate
        .endif
mov _lpRecordingIndexBuf,eax
invoke VirtualAlloc,_lpRecordingIndexBuf,_dwRecordingIndexBufSize,MEM_COMMIT,PAGE_READWRITE
        .if eax==0
        invoke WriteFile,_hFileJournal,addr szMemAllocError,sizeof szMemAllocError-1,addr @dwCounter,0
        call _ProcTerminate
        .endif
...
invoke VirtualAlloc,_lpIndexReservedBuf,_dwRecordingIndexBufSize,MEM_COMMIT,PAGE_READWRITE
        .if eax==0
        invoke WriteFile,_hFileJournal,addr szMemAllocError,sizeof szMemAllocError-1,addr @dwCounter,0
        call _ProcTerminate
        .endif
...
invoke CreateThread,0,0,ebx,0,CREATE_SUSPENDED,0
        .if eax==0
...
invoke ResumeThread,_hThreadRealTimeData
...
;1个点2个OVERLAPPED结构，每个OVERLAPPED结构32字节,以1024个采样点为单位
...
;local @lpQueryDataBuf
;local @dqDataPointer:qword
;local @dqRecordHeadPointer:qword               ;<---参数入口指针
...
mov @dwOverlappedInfoSize,4000h
...
;检测OVERLAPPED状态，如果读完则取出放到缓冲区并发送下一个读请求，如果错误退出，如果没读完检测下一个
...
mov dword ptr [esi],0
mov dword ptr [esi+4],0
jmp SendReadTimeOverlapped

CheckTimeOverlappedEnd:
add @lpOverlappedInfoPtr,32

CheckValueOverlappedStaus:
mov eax,@dwReadValueSectorCnt
cmp eax,@dwReadRecordSum
jnz ValueCntLess
mov eax,@dwReadTimeSectorCnt
...
mov dword ptr [esi],0
mov dword ptr [esi+4],0
jmp SendReadValueOverlapped

CheckValueOverlappedEnd:
add @lpOverlappedInfoPtr,32
mov esi,@lpOverlappedInfoPtr
...
mov eax,_lpRecordingBufHead             ;保存固定地址，实时值计算地址可以直接计算
...
cmp dword ptr _flWriteBusying,1
jz ToWaitTime
mov eax,dword ptr [_dqRecordingCntSum]
mov edx,dword ptr [_dqRecordingCntSum+4]
...
;异常处理函数是由WINDOWS调用的，而且调用的时候是C格式调用，因此由WINDOWS负责恢复堆栈，所以RET返回时不必从堆栈弹出参数
...
mov eax,[esi].ExceptionCode
        ...
        pop edi
        pop esi
        invoke wsprintf,addr @szBuf,addr szGuardPageViolation,addr @szBuffer,[esi].ExceptionAddress,[esi].ExceptionCode,\
        [esi].ExceptionFlags   
        ...
        AccessViolation:
        ;invoke SuspendThread,_hThreadWaitSamplePeriod     ;加上这两个函数就卡住了，为什么??????????
        ;invoke SuspendThread,_hThreadUdpUnpack
        mov esi,lpExceptionRecord
        mov edi,lpContext              
        assume esi:ptr EXCEPTION_RECORD,edi:ptr CONTEXT
        ...
        pop edi
        pop esi
        invoke wsprintf,addr @szBuf,addr szMsgException,addr @szBuffer,[esi].ExceptionAddress,[esi].ExceptionCode,\
        [esi].ExceptionFlags   
        assume esi:nothing,edi:nothing
        mov ecx,eax
        invoke WriteFile,_hFileJournal,addr @szBuf,ecx,addr @dwCounter,0
        cmp _flMemoryNotEnough,1
        jz RequestMemoryError

        mov ebx,_lpRecordingBuf
        add ebx,_dwRecordingBufSize
        mov ecx,_dwRecordingBackupBufSize
        sub ecx,_dwRecordingBufSize
        invoke VirtualAlloc,ebx,ecx,MEM_COMMIT,PAGE_READWRITE
        mov ebx,_lpRecordingIndexBuf
        add ebx,_dwRecordingIndexBufSize
        mov ecx,_dwIndexBackupBufSize
        sub ecx,_dwRecordingIndexBufSize
        invoke VirtualAlloc,ebx,ecx,MEM_COMMIT,PAGE_READWRITE                    
        invoke SetThreadPriority,_hThreadStoreRecording,THREAD_PRIORITY_HIGHEST
        mov _flReadWriteBusy,1
        mov _flMemoryNotEnough,1
        invoke WriteFile,_hFileJournal,addr szExceptionReadWriteBusy,sizeof szExceptionReadWriteBusy-1,addr @dwCounter,0
        ;invoke ResumeThread,_hThreadWaitSamplePeriod   ;这两个函数不会得到执行，为什么？？？？？？？？？？
        ;invoke ResumeThread,_hThreadUdpUnpack
        popad
        mov eax,ExceptionContinueExecution
        leave
        retn 0
        RequestMemoryError:
        mov ebx,_lpRecordingBuf
        lock xchg dword ptr _lpRecordingBufHead,ebx
        mov edx,_lpRecordingIndexBuf
        lock xchg dword ptr _lpRecordingIndexBufHead,edx
        invoke SetThreadPriority,_hThreadStoreRecording,THREAD_PRIORITY_HIGHEST
        mov _flReadWriteBusy,1
        invoke WriteFile,_hFileJournal,addr szExceptionMemUnroll,sizeof szExceptionMemUnroll-1,addr @dwCounter,0
        popad
        mov eax,ExceptionContinueExecution
        leave
        retn 0
                
        .elseif eax==EXCEPTION_IN_PAGE_ERROR  ;0C0000006H               
        jmp AccessViolation
